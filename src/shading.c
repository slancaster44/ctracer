...
Tuple3 StepShading(Material m, Light l, Tuple3 position, Tuple3 eye_vector, Tuple3 surface_normal, float step) {
    Tuple3 effective_color = TupleMultiply(m.color, l.color);
    Tuple3 ambient = TupleScalarMultiply(effective_color, m.ambient_reflection);
    ...
    Tuple3 output = TupleAdd(diffuse, specular);
    float trueStep = 0;
    for (int i = 0; i < 4; i++) {
        trueStep += fmodf(output[i], step);
    }
    
    output = TupleScalarSubtract(output, trueStep / 4);
    
    return TupleAdd(ambient, output);
}